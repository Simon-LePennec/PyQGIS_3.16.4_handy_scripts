...
@alg(name='save_in_4326_geojson', label='Reproject in 4326 and save layer as geojson (6 decimals) with _4326 suffix',
...
def savein4326geojson(instance, parameters, context, feedback, inputs):
    """
    Reproject input in 4326 and save output as a geojson (6 decimals) with _4326 suffix, in the input's folder.
    You don't need to select output location.
    The purpose of this is to save time while working on webmapping, because reprojections are pretty frequent.
    EPSG:4326 is the default coordinate system for MapboxGL.js
    """
    ...
    #output_layer_path = input_layer_path[:-4]+'_4326.geojson'
    ...
    new_filename = filename.split("'")[0][:-4]+'_4326.geojson'
    ...
    #For datavisualisations we don't need more than 6 decimals
